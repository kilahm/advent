export function incompleteErrorScore(line: string): number {
  ...
  const errorChar = line
    ...
    if(errorChar !== undefined || opens.length === 0) {
      return 0
    }

    return opens.reduceRight((acc, open) => acc * 5 + OpenCharScore[open],0);
}
